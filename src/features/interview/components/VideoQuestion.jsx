...
export default function VideoQuestion({
  ...
  const requestCamera = async () => {
    try {
      ...
      alert(t("interview_page.video_question.camera_required_alert"));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-5 py-12">
        <div className="size-16 rounded-full bg-[--stage-interview]/10 flex items-center justify-center">
          <svg
            className="size-8 text-[--stage-interview]"
            fill="none"
            stroke="currentColor"
            strokeWidth={1.5}
            viewBox="0 0 24 24"
        ...
        <div className="text-center space-y-1">
          <p className="text-sm font-medium text-foreground">
            {t("interview_page.video_question.camera_required_title")}
          </p>
          <p className="text-xs text-muted-foreground">
            {t("interview_page.video_question.camera_required_desc")}
          </p>
        </div>
        <button
          onClick={requestCamera}
          className="bg-primary text-primary-foreground rounded-lg px-4 py-2.5 text-sm font-medium hover:opacity-90 transition-opacity"
        >
          {t("interview_page.video_question.enable_camera")}
        </button>
      </div>
    );
  }
      ...
      <div className="space-y-2">
        {status === "previewing" && (
          ...
          >
            ...
            {t("interview_page.video_question.start_recording")}
          </button>
        )}
        ...
        {status === "recording" && (
          ...
          >
            ...
            {t("interview_page.video_question.stop_recording")}
          </button>
        )}
