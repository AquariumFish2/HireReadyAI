...
export default function ItemDialog({ open, onClose, title, children, onSave, saving }) {
  if (!open) return null;
      ...
      <div className="relative bg-background rounded-2xl border border-border/60 shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border/60 shrink-0">
          ...
          <button
            onClick={onSave}
            disabled={saving}
            className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-primary-foreground bg-primary rounded-lg hover:bg-primary/90 disabled:opacity-50 transition-colors"
          >
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
