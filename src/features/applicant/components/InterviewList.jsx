...
export default function InterviewList({ applications }) {
  ...
  const filteredInterviews = interviewProcesses.filter((app) => {
    ...
    if (activeTab === "interview")
      return activeStage || app.current_stage === APPLICATION_STAGE.interview;
    if (activeTab === "completed")
      return !activeStage && app.current_stage === APPLICATION_STAGE.hired;
    if (activeTab === "rejected")
      return !activeStage && app.current_stage === APPLICATION_STAGE.rejected;
    return true;
  });
        ...
        {filteredInterviews.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "9px" }}>
            {filteredInterviews.map((app) => {
              ...
              // let displayLabel = app.current_stage;
              // let stageCfg = stageConfig[app.current_stage] || defaultStage;

              // if (activeStage) {
              //   const type = activeStage.recruitment_stages?.stage_type;
              //   displayLabel =
              //     activeStage.recruitment_stages?.name || "Interview Stage";
              //   stageCfg = stageConfig[type] || defaultStage;
              // } else if (stageConfig[app.current_stage]) {
              //   // displayLabel = stageConfig[app.current_stage].label;
              //   const cfg = stageConfig[type] || defaultStage;

              //   const displayLabel = t(cfg.labelKey);
              // }
              let displayLabel = "";
              let stageCfg = defaultStage;
              ...
              if (activeStage) {
                ...
                displayLabel =
                  activeStage.recruitment_stages?.name || t(stageCfg.labelKey);
              } else {
                      ...
                      >
                        ...
                        {activeStage
                          ? t("interview_list.buttons.start_stage_interview", {
                              stage: activeStage.recruitment_stages?.name,
                            })
                          : t("interview_list.buttons.start_ai_interview")}
                      </button>
                    )}
