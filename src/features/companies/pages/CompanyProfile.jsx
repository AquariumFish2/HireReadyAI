  ...
  return (
    <div>
      ...
      <input
        ...
        className={`w-full h-9 px-3 border rounded-lg text-sm font-medium ${
          editing
    ...
    <div>
      ...
      {editing ? (
        ...
        <input
          ...
          disabled
          value=""
          className="w-full h-9 px-3 border border-border rounded-lg text-sm bg-muted text-muted-foreground font-medium"
        />
      )}
    </div>
  );
...
export default function CompanyProfile({
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      const updated = await updateCompany(company.id, { logo_url: logoUrl });
  ...
  const handleCoverUpload = async (e) => {
    ...
    try {
      ...
      const updated = await updateCompany(company.id, { cover_url: coverUrl });
      onCompanyUpdate(updated);
    } catch (err) {
  ...
  const permissionBadge = (perm) => {
    const styles = {
      ...
      [MEMBERSHIP_PERMISSION.pending]:
        "bg-warning/10 text-warning border-warning/20",
    };
    return (
      <span
        className={`px-2 h-5 flex items-center rounded-md text-[10px] font-bold border shrink-0 ${
          styles[perm] || "bg-muted text-muted-foreground border-border"
        }`}
      >
        ...
        >
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex items-center gap-4">
              <div
                className="relative w-16 h-16 -mt-10 rounded-xl overflow-hidden shrink-0 group cursor-pointer border-2 border-background shadow-xs"
          ...
          {editing && (
            <div className="mb-4 p-3 bg-primary/5 border border-primary/20 rounded-lg text-xs text-primary">
              You are editing company details
            </div>
          )}
          ...
          <div dir="ltr" className="space-y-4">
            ...
            <div>
              ...
              {editing ? (
                ...
                <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">
                  {company?.culture || "No culture info provided."}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              {editing ? (
                ...
                <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">
                  {company?.benefits || "No benefits info provided."}
                </p>
              )}
            </div>
          ...
          {isHrManager && (
            ...
            >
              <input
                ...
                className="w-full sm:flex-1 h-9 px-3 bg-background border border-border rounded-lg text-xs text-foreground font-medium focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary transition-all"
              />
              <input
                required
                type="email"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        className="p-1.5 bg-success/20 text-success rounded-md hover:bg-success/30 transition-colors cursor-pointer"
                        title="Accept"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 bg-destructive/20 text-destructive rounded-md hover:bg-destructive/30 transition-colors cursor-pointer"
                        title="Reject"
                      >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {(isHrManager && member.profile_id !== currentUserId) ? (
                    <select
                      ...
                      onChange={(e) =>
                        handleChangePermission(member.id, e.target.value)
                      }
                      className="px-2 h-6 rounded-md text-[10px] font-bold border bg-background text-foreground border-border cursor-pointer focus:outline-none focus:ring-1 focus:ring-ring"
                    >
                      <option value={MEMBERSHIP_PERMISSION.recruiter}>
                        Recruiter
                      </option>
                      <option value={MEMBERSHIP_PERMISSION.hrManager}>
                        HR Manager
                      </option>
                    </select>
                  ) : (
