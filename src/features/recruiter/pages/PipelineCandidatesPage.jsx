...
const scoreColor = (s) => {
  ...
  if (s >= 70) return "bg-violet-100 text-violet-700";
  if (s >= 55) return "bg-amber-100 text-amber-700";
  ...
  return (
    <div
      className={`flex flex-col min-w-[270px] w-[270px] shrink-0 rounded-2xl transition-all duration-200 ${
        isOver && !isLocked ? "ring-2 ring-dark-amethyst-400 ring-offset-2" : ""
      }`}
      onDragOver={(e) => {
        e.preventDefault();
    ...
    >
      ...
      <div
        className={`flex-1 overflow-y-auto space-y-3 px-1 pb-4 pr-2 transition-all duration-200 ${
          isOver && !isLocked ? "bg-dark-amethyst-50/60 rounded-xl" : ""
        }`}
      ...
      >
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 max-w-sm relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-dark-amethyst-300" />
            <input
              ref={searchRef}
              value={search}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-semibold transition ${
                showFilters
                  ...
                  : "border-dark-amethyst-200 text-dark-amethyst-600 bg-white hover:bg-dark-amethyst-50"
              }`}
            >
