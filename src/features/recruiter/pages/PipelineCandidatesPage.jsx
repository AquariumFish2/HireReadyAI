...
const scoreColor = (s) => {
  if (s >= 85) return "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20";
  if (s >= 70) return "bg-primary/10 text-primary border border-primary/20";
  if (s >= 55) return "bg-amber-500/10 text-amber-600 border border-amber-500/20";
...
function getFit(score, isRejected) {
  ...
  return { label: "Low Fit", cls: "bg-destructive/10 text-destructive border-destructive/20" };
}

const CandidateCard = ({ candidate, onDragStart, isDragging }) => {
  const fit = getFit(candidate.score, candidate.is_rejected);
  return (
    <div
      draggable
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {candidate.score > 0 && (
          <span className={`px-2 py-0.5 rounded-lg text-xs font-bold font-display ${scoreColor(candidate.score)}`}>
            {candidate.score}
          </span>
        )}
      </div>
      ...
      {candidate.score > 0 && (
        <div>
          ...
          <div className="w-full h-1.5 rounded-full bg-secondary overflow-hidden border border-border/50">
            <div
              className={`h-full rounded-full transition-all duration-700 ${candidate.score >= 85
                  ? "bg-emerald-500"
                  : candidate.score >= 70
                    ? "bg-primary"
                    : candidate.score >= 55
...
const PipelineColumn = ({
  ...
  return (
    <div
      className={`flex flex-col min-w-[270px] w-[270px] shrink-0 rounded-2xl transition-all duration-200 ${isOver && !isLocked ? "ring-2 ring-primary ring-offset-2 bg-secondary/20" : ""
        }`}
      onDragOver={(e) => {
        e.preventDefault();
    ...
    >
      ...
      <div
        className={`flex-1 overflow-y-auto space-y-3 px-1 pb-4 pr-2 transition-all duration-200 ${isOver && !isLocked ? "bg-secondary/40 rounded-xl" : ""
          }`}
        style={{ maxHeight: "calc(100vh - 230px)", minHeight: 120 }}
      >
        {candidates.length === 0 ? (
          <div
            className={`rounded-xl border-2 border-dashed h-24 flex items-center justify-center transition-all ${isLocked
...
export default function PipelineCandidatesPage({ company, jobs = [] }) {
  ...
  const filtered = candidates.filter((c) => {
    ...
    if (filterFit !== "All" && getFit(c.score, c.is_rejected).label !== filterFit)
      return false;
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-semibold transition-all ${showFilters
          ...
          <div className="mt-4 pt-4 border-t border-border flex items-center gap-3 flex-wrap">
            ...
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${filterFit === f
                    ? "bg-primary text-white border-primary shadow-sm"
